$(function(){
  ...
  //配送方法（出品者負担）
  function appendMethod_1(method, default_d_method) {
    var method = `<div class= 'sizeform-box margin-top-40' id= 'method_box'>
                    ...
                    <div class='select-box'>
                      ...
                      <select id="input-method" class="select-box--menu choosing d_method1" name="product[d_method]">
                        <option value="${default_d_method}" selected class="delete_list">${default_d_method}</option>
                        <option value="---">---</option>
  ...
  //配送方法（購入者負担）
  function appendMethod_2(method, default_d_method) {
    var method = `<div class= 'sizeform-box margin-top-40' id= 'method_box'>
                    <label>配送の方法
                      <span class="require">必須
                      </span>
  ...
  $(document).on('turbolinks:load', function(){
    ...
    if (deliveryName == "送料込み(出品者負担)" ){
      ...
      appendMethod_1(method, default_d_method)
      $('.d_method1').on('click', function(){
        $('.delete_list').remove();
      });
    }else if (deliveryName == "着払い(購入者負担)" ){
      var default_d_method = $('.product-d_method').val();
      appendMethod_2(method, default_d_method)
      $('.d_method2').on('click', function(){
        $('.delete_list').remove();
      });
    };
  });
